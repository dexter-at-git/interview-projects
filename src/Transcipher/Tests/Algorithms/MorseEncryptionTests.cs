using System;
using Transcipher.Algorithms;
using Transcipher.Controllers;
using Xunit;

namespace Transcipher.Tests.Algorithms
{
    public class MorseEncryptionTests
    {
        public IEncryptionAlgorithm _morseAlgorithm = new MorseEncryption();

        [Fact]
        public void Encrypt_EmptyInput_ShouldReturnEmptyString()
        {
            var text = String.Empty;

            var encryptedData = _morseAlgorithm.Encrypt(text);

            Assert.Equal(String.Empty, encryptedData);
        }

        [Fact]
        public void Decrypt_EmptyInput_ShouldReturnEmptyString()
        {
            var text = String.Empty;

            var decryptedData = _morseAlgorithm.Decrypt(text);

            Assert.Equal(String.Empty, decryptedData);
        }

        [Fact]
        public void Encryption_Decryption_Correctness()
        {
            var text = "TEST message For encryption 1234";

            var encryptedData = _morseAlgorithm.Encrypt(text);
            var decryptedData = _morseAlgorithm.Decrypt(encryptedData);

            Assert.Equal(text, decryptedData, true);
        }
    }
}
